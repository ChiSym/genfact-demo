# source https://github.com/OxygenFramework/Oxygen.jl/blob/master/src/extensions/templating/mustache.jl
using HTTP
using MIMEs
using Mustache

"""
    mustache_(template::String; kwargs...)
...
"""
function mustache_(template::String; mime_type=nothing, from_file=false, kwargs...)
    mime_is_known = !isnothing(mime_type)

    # Case 1: a path to a file was passed
    if from_file
        if mime_is_known
            return open(io -> mustache_(io; mime_type=mime_type, kwargs...), template)
        else
            ...
            return open(io -> mustache_(io; mime_type=content_type, kwargs...), template)
        end
    end
...
"""
    mustache_(tokens::Mustache.MustacheTokens; kwargs...)
...
To get more info read the docs here: https://github.com/jverzani/Mustache.jl
"""
function mustache_(tokens::Mustache.MustacheTokens; mime_type=nothing, kwargs...)
    mime_is_known = !isnothing(mime_type)
...
To get more info read the docs here: https://github.com/jverzani/Mustache.jl
"""
function mustache_(file::IO; mime_type=nothing, kwargs...)
