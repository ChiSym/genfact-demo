...
const _tokenizer_config = open(_tokenizer_config_path) do file
    JSON3.read(read(file, String))
end
const _template_params = Dict("bos_token" => _tokenizer_config["bos_token"])
const _chat_template = _tokenizer_config["chat_template"]
const _jinja2 = pyimport("jinja2")
const _jinja2sandbox = pyimport("jinja2.sandbox")
const _jinja2exceptions = pyimport("jinja2.exceptions")
...
const _compiled_chat_template = begin
    py"""
import json
...
"""
    ...
    env.from_string(_chat_template)
end
