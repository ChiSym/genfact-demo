@testset ExtendedTestSet "building query" begin
    resolve(table, symb) = GenFactDemo.PClean.resolve_dot_expression(GenFactDemo.MODEL, table, symb)
...
# PClean Query Tests
#####################
const PHYSICIAN_RESPONSE_ATTRIBUTES = Set(["npi", "first", "last", "degree", "school", "specialty"])
const BUSINESS_RESPONSE_ATTRIBUTES = Set(["addr", "addr2", "zip", "city", "legal_name"])

function verify_entry(pclean_triplet, expected_physician_attributes, expected_business_attributes)
...
function verify_pclean_results(pclean_results, expected_physician_attributes=Dict{String, String}(), expected_business_attributes=Dict{String,String}())
    @test Set(("physician_histogram", "results", "business_histogram")) == keys(pclean_results)
    ...
    verify_entry.(results, Ref(expected_physician_attributes), Ref(expected_business_attributes))
end
